...
class KinovaTestEnv(RCareWorld):
    ...
    # Objects
    _plate_id: int = 800001
    _fork_id: int = 800002
    _spoon_id: int = 800003
    _knife_id: int = 800004
    _cup_id: int = 800005
    _napkin_id: int = 800006
    _banana_id: int = 800011
    _peach_id: int = 800012
    ...
    def __init__(
        ...
        **kwargs
        ...
        if executable_file == "@editor":
            print("\n" + "="*70)
            print("[Unity Editor Mode] Feeding Environment")
            print("="*70)
    ...
    def get_plate(self):
        """Get plate object."""
        try:
            return self.GetAttr(self._plate_id)
        except AssertionError:
            print(f"Error: Plate with ID {self._plate_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._plate_id}")
            raise

    def get_fork(self):
        """Get fork object."""
        try:
            return self.GetAttr(self._fork_id)
        except AssertionError:
            print(f"Error: Fork with ID {self._fork_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._fork_id}")
            raise

    def get_spoon(self):
        """Get spoon object."""
        try:
            return self.GetAttr(self._spoon_id)
        except AssertionError:
            print(f"Error: Spoon with ID {self._spoon_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._spoon_id}")
            raise
    
    def get_knife(self):
        """Get knife object."""
        try:
            return self.GetAttr(self._knife_id)
        except AssertionError:
            print(f"Error: Knife with ID {self._knife_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._knife_id}")
            raise

    def get_cup(self):
        """Get cup object."""
        try:
            return self.GetAttr(self._cup_id)
        except AssertionError:
            print(f"Error: Cup with ID {self._cup_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._cup_id}")
            raise

    def get_napkin(self):
        """Get napkin object."""
        try:
            return self.GetAttr(self._napkin_id)
        except AssertionError:
            print(f"Error: Napkin with ID {self._napkin_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._napkin_id}")
            raise

    def get_banana(self):
        """Get banana object."""
        try:
            return self.GetAttr(self._banana_id)
        except AssertionError:
            print(f"Error: Banana with ID {self._banana_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._banana_id}")
            raise

    def get_peach(self):
        """Get peach object."""
        try:
            return self.GetAttr(self._peach_id)
        except AssertionError:
            print(f"Error: Peach with ID {self._peach_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._peach_id}")
            raise

    def get_strawberry1(self):
        """Get strawberry1 object."""
        try:
            return self.GetAttr(self._strawberry1_id)
        except AssertionError:
            print(f"Error: Strawberry1 with ID {self._strawberry1_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._strawberry1_id}")
            raise

    def get_strawberry2(self):
        """Get strawberry2 object."""
        try:
            return self.GetAttr(self._strawberry2_id)
        except AssertionError:
            print(f"Error: Strawberry2 with ID {self._strawberry2_id} not found")
            print(f"Make sure the object exists in Unity with Instance ID = {self._strawberry2_id}")
            raise
    
    def get_strawberry3(self):
        """Get strawberry3 object."""
        try:
            return self.GetAttr(self._strawberry3_id)
        except AssertionError:
            print(f"Error: Strawberry3 with ID {self._strawberry3_id} not found")
    ...
    def print_scene_info(self):
        ...
        print(f"Plate ID:            {self._plate_id}")
        print(f"Fork ID:             {self._fork_id}")
        print(f"Spoon ID:            {self._spoon_id}")
        print(f"Knife ID:            {self._knife_id}")
        print(f"Cup ID:              {self._cup_id}")
        print(f"Napkin ID:           {self._napkin_id}")
        print(f"Banana ID:           {self._banana_id}")
        print(f"Peach ID:            {self._peach_id}")
